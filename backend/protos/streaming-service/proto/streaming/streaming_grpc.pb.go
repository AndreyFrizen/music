...
const (
	StreamingService_StreamTrack_FullMethodName = "/streaming.StreamingService/StreamTrack"
)
...
type StreamingServiceClient interface {
	...
	StreamTrack(ctx context.Context, in *StreamRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Chunk], error)
}
...
func (c *streamingServiceClient) StreamTrack(ctx context.Context, in *StreamRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Chunk], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &StreamingService_ServiceDesc.Streams[0], StreamingService_StreamTrack_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type StreamingService_StreamTrackClient = grpc.ServerStreamingClient[Chunk]
...
type StreamingServiceServer interface {
	// Получение аудио-потока для стриминга
	StreamTrack(*StreamRequest, grpc.ServerStreamingServer[Chunk]) error
...
func (UnimplementedStreamingServiceServer) StreamTrack(*StreamRequest, grpc.ServerStreamingServer[Chunk]) error {
	return status.Error(codes.Unimplemented, "method StreamTrack not implemented")
}
...
func _StreamingService_StreamTrack_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StreamRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(StreamingServiceServer).StreamTrack(m, &grpc.GenericServerStream[StreamRequest, Chunk]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type StreamingService_StreamTrackServer = grpc.ServerStreamingServer[Chunk]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _StreamingService_StreamTrack_Handler,
			ServerStreams: true,
		},
	},
